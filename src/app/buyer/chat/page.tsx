...
import { MessageSquare, Send, ShoppingBag, ShoppingCart, MapPin } from "lucide-react";
  ...
  return (
    <DashboardLayout sidebarLinks={buyerLinks} sidebarTitle="Buyer Panel">
      ...
      {loading ? (
        ...
        <EmptyState icon={<MessageSquare className="h-12 w-12" />} title="No conversations yet" description="Start a chat from a product page." />
      ) : (
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {messages.map((msg: any) => (
                    <div key={msg.id} className="flex flex-col gap-1">
                      <div className="bg-muted rounded-lg px-3 py-2 max-w-xs text-sm">
                        {msg.content}
                      </div>
