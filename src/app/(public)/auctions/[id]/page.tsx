...
export default function AuctionDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleBid = async () => {
    ...
    if (!amount || isNaN(amount)) {
      setBidMsg({ text: "Enter a valid bid amount", success: false });
      return;
    }
  ...
  return (
    <PageContainer className="py-8">
      {/* Breadcrumb — note: uses /auctions (plural, correct route) */}
            ...
            <div className="flex justify-between items-center">
              ...
              <span
                className={`font-medium ${
                  isEnded ? "text-muted-foreground" : "text-destructive"
                }`}
              >
                {timeLeft}
              </span>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {auction.bids.map((bid: any, i: number) => (
                  <tr
                    key={bid.id}
                    className={i === 0 ? "bg-green-50" : "hover:bg-muted/30"}
                  >
