...
import {
  CheckCircle, Package, ShoppingBag, Gavel, MessageSquare,
  BarChart2, User, BadgeCheck, Trash2,
} from "lucide-react";
...
export default function NewProductPage() {
  ...
  // Image state — upload after product is created
  ...
  const {
    ...
    defaultValues: { condition: "new", listingType: "fixed", unit: "kg" },
  });
  ...
  const onSubmit = async (data: ProductInput) => {
    ...
    if (json.success) {
      // FIX: store ID, show image uploader — don't redirect yet
      setCreatedProductId(json.data.id);
      setSaved(true);
    } else {
      setServerError(json.message || "Failed to create product");
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // FIX: Do NOT set Content-Type header — browser sets multipart boundary automatically
    Array.from(files).forEach((f) => form.append("image", f));
    try {
      const res = await fetch(`/api/products/${createdProductId}/images`, { method: "POST", body: form });
      const json = await res.json();
      if (json.success) {
        const refreshed = await fetch(`/api/products/${createdProductId}/images`).then((r) => r.json());
        if (refreshed.success) setImages(refreshed.data);
      } else {
        setImageError(json.message || "Upload failed.");
      }
    } catch {
  ...
  const handleDeleteImage = async (imageId: string) => {
    if (!createdProductId) return;
    await fetch(`/api/products/${createdProductId}/images/${imageId}`, { method: "DELETE" });
    setImages((prev) => prev.filter((img) => img.id !== imageId));
  };

  const handleSetPrimary = async (imageId: string) => {
    if (!createdProductId) return;
    await fetch(`/api/products/${createdProductId}/images/${imageId}`, { method: "PATCH" });
  ...
  return (
    <DashboardLayout sidebarLinks={sellerLinks} sidebarTitle="Seller Panel">
      ...
      <div className="max-w-2xl space-y-6">
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea {...register("description")} rows={4} placeholder="Describe your product..." className="px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none" />
              </div>

              {saved && (
                <div className="flex items-center gap-2 text-green-700 bg-green-50 border border-green-200 px-3 py-2 rounded-md text-sm">
                  <CheckCircle className="h-4 w-4" /> Product created! Add images below, then click Done.
                </div>
              )}
              {serverError && <p className="text-sm text-destructive bg-destructive/10 px-3 py-2 rounded-md">{serverError}</p>}

              {/* Hide submit once product is created */}
              {!createdProductId && (
        ...
        {/* Image upload — shown only after product is created */}
        {createdProductId && (
          <Card>
            <CardContent className="p-6 space-y-4">
              <h3 className="font-semibold">Product Images</h3>
              <p className="text-sm text-muted-foreground">Upload photos. The first image becomes the primary image.</p>

              {imageError && <p className="text-sm text-destructive bg-destructive/10 px-3 py-2 rounded-md">{imageError}</p>}

              <div className="flex flex-wrap gap-3">
                {images.map((img) => (
                  <div key={img.id} className="relative group">
                    <img src={img.url} alt="" className="h-20 w-20 object-cover rounded-lg border" />
                    {img.isPrimary && <span className="absolute top-1 left-1 bg-primary text-white text-xs px-1 rounded">Primary</span>}
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-1">
                      {!img.isPrimary && (
                        <button onClick={() => handleSetPrimary(img.id)} className="text-white text-xs bg-primary px-1.5 py-0.5 rounded">Set Primary</button>
                      )}
                      <button onClick={() => handleDeleteImage(img.id)} className="text-white"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
                ...
                <label className="h-20 w-20 border-2 border-dashed border-input rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-primary transition-colors">
                  ...
                  <span className="text-xs text-muted-foreground">{imageUploading ? "Uploading" : "Add"}</span>
                  <input type="file" accept="image/*" multiple className="hidden" onChange={handleImageUpload} disabled={imageUploading} />
                </label>
              </div>

              <p className="text-xs text-muted-foreground">Accepted: JPG, PNG, WEBP. Max 5MB per image.</p>

              {/* FIX: Go to products list, not edit page */}
              <div className="flex gap-3 pt-2">
                <Button onClick={() => router.push("/seller/products")}>Done — Go to My Products</Button>
                <Button variant="outline" onClick={() => router.push(`/seller/products/${createdProductId}/edit`)}>Continue Editing</Button>
              </div>
            </CardContent>
          </Card>
        )}
