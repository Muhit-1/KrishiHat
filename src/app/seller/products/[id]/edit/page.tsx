...
const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  try {
    ...
    if (json.success) {
      ...
      setImageError(json.message || "Upload failed. Please try again.");
    }
  } catch {
    setImageError("Network error during upload. Please try again.");
  } finally {
  
    if (uploadSucceeded) {
      try {
        const refreshed = await fetch(`/api/products/${params.id}/images`).then((r) =>
          r.json()
        );
  ...
  const handleDeleteImage = async (imageId: string) => {
    await fetch(`/api/products/${params.id}/images/${imageId}`, {
      method: "DELETE",
    });
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-1">
                    {!img.isPrimary && (
                      <button
                        ...
                        className="text-white text-xs bg-primary px-1.5 py-0.5 rounded"
                      >
                        Set Primary
                      </button>
                    )}
            ...
            <p className="text-xs text-muted-foreground">
              Accepted: JPG, PNG, WEBP. Max 5MB per image.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              />
              <Input
                label="Product Title (Bengali)"
                placeholder="বাংলায় নাম (ঐচ্ছিক)"
                {...register("titleBn")}
              />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    ...
                    {selectedCategory?.auctionAllowed && (
